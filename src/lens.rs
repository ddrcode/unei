...
// ----------------------------------------------------------------------
// 6502 / 65C02 disassembler (#65)
// ----------------------------------------------------------------------

/// Every base mnemonic the opcode table knows (the bit ops RMB/SMB/BBR/BBS
/// are generated by suffix, below). Shared by the disassembler's inverse
/// table and the opcode tests, so both walk the same set.
...
/// The bit-addressed 65C02 mnemonics (`RMB0`..`BBS7`), generated from the
/// four bases the opcode table encodes by `base + bit*$10`.
pub(crate) const BIT_OP_BASES: [&str; 4] = ["RMB", "SMB", "BBR", "BBS"];

/// The inverse of the opcode table: opcode byte → (mnemonic, decoding).
/// Built once from the forward table (`opcode_byte`), so it can never
/// disagree with it — the same iteration the uniqueness test walks. Bytes
/// with no 65C02 instruction stay `None` (emitted as raw `!byte`).
fn decode_table() -> &'static [Option<(&'static str, Op)>; 256] {
    static TABLE: std::sync::OnceLock<[Option<(&'static str, Op)>; 256]> =
        std::sync::OnceLock::new();
    TABLE.get_or_init(|| {
        let mut t: [Option<(&'static str, Op)>; 256] = [None; 256];
        let mut fill = |mn: &'static str| {
            if let Some(ops) = ops_for(mn) {
                for o in ops {
                    if let Some(b) = opcode_byte(mn, o.mode) {
                        t[b as usize] = Some((mn, *o));
                    }
                }
            }
        };
        ...
        // RMB0..SMB7/BBR0..BBS7 — leak the 32 generated names once so the
        // table can hold `&'static str` like the base mnemonics.
        for base in BIT_OP_BASES {
            for d in 0..8 {
                let name: &'static str = Box::leak(format!("{base}{d}").into_boxed_str());
                fill(name);
            }
        }
        t
...
/// Formats the operand of one decoded instruction. `bytes` is the whole
/// instruction (opcode first); `addr` is where it sits, for relative
/// branch targets. Output is ACME source syntax.
fn format_operand(mode: Mode, bytes: &[u8], addr: u16) -> String {
    ...
    match mode {
        ...
        Mode::ZpInd => format!("(${b1:02x})"),
        Mode::Ind => format!("(${word:04x})"),
        Mode::IndAbsX => format!("(${word:04x},x)"),
        // branch offset is signed, relative to the byte after the instruction
        Mode::Rel => {
...
/// Disassembles a 65C02 code image into re-assemblable ACME source, one
/// instruction per line with an address+bytes comment. The leading
/// `; asm: 65c02 acme` modeline makes the result highlight and answer the
/// machine lens (`K`) like any hand-written `.s` file — the opcode table
/// that decoded the bytes then annotates its own output.
///
/// `org` is the load address; `total` and `name` only feed the header.
pub fn disassemble(code: &[u8], org: u16, name: &str, total: usize) -> String {
    let table = decode_table();
    let mut out = String::new();
    out.push_str("; asm: 65c02 acme\n");
    out.push_str(&format!(
        "; disassembly of {name} — {total} bytes, load ${org:04x}\n\n"
    ));
    ...
    while i < code.len() {
        ...
        match table[byte as usize] {
            Some((mn, o)) => {
                let len = o.mode.bytes() as usize;
                if i + len > code.len() {
                    // an instruction runs off the end of the image: the
                    // trailing bytes are data, not a truncated opcode
                    for &b in &code[i..] {
                        out.push_str(&format!(
                            "        !byte ${b:02x}              ; ${pc:04x}\n"
                        ));
                ...
                let instr = &code[i..i + len];
                let operand = format_operand(o.mode, instr, pc);
                let raw: Vec<String> = instr.iter().map(|b| format!("{b:02x}")).collect();
                let mnem = mn.to_ascii_lowercase();
            ...
            None => {
                out.push_str(&format!(
                    "        !byte ${byte:02x}              ; ${pc:04x}  (undefined)\n"
                ));
                pc = pc.wrapping_add(1);
                i += 1;
            }
...
mod tests {
    ...
    #[test]
    fn decode_table_is_the_exact_inverse_of_the_opcode_table() {
        // every entry the disassembler will emit re-encodes to its own byte:
        // the inverse can't drift from the forward table it was built from
        let mut n = 0;
        for (byte, entry) in decode_table().iter().enumerate() {
            if let Some((mn, o)) = entry {
                assert_eq!(
                    opcode_byte(mn, o.mode),
                    Some(byte as u8),
                    "byte ${byte:02X} decodes to {mn} {:?} but that re-encodes elsewhere",
                    o.mode
                );
                n += 1;
            }
        }
        assert!(n > 140, "only {n} opcodes decodable");
    }

    #[test]
    fn operand_formats_match_acme_syntax() {
        assert_eq!(format_operand(Imm, &[0xA9, 0x2A], 0), "#$2a");
        assert_eq!(format_operand(Zp, &[0xA5, 0x10], 0), "$10");
        assert_eq!(format_operand(ZpX, &[0xB5, 0x10], 0), "$10,x");
        assert_eq!(format_operand(Abs, &[0xAD, 0x34, 0x12], 0), "$1234");
        assert_eq!(format_operand(AbsY, &[0xB9, 0x34, 0x12], 0), "$1234,y");
        assert_eq!(format_operand(IndX, &[0xA1, 0x20], 0), "($20,x)");
        assert_eq!(format_operand(IndY, &[0xB1, 0x20], 0), "($20),y");
        assert_eq!(format_operand(ZpInd, &[0xB2, 0x20], 0), "($20)");
        assert_eq!(format_operand(Ind, &[0x6C, 0x34, 0x12], 0), "($1234)");
        assert_eq!(format_operand(Acc, &[0x0A], 0), "");
        // relative targets are signed and measured from the next instruction
        assert_eq!(format_operand(Rel, &[0xD0, 0x05], 0xC000), "$c007");
        assert_eq!(format_operand(Rel, &[0xD0, 0xFB], 0xC000), "$bffd"); // -5
        assert_eq!(
            format_operand(ZpRel, &[0x0F, 0x12, 0x08], 0xC000),
            "$12,$c00b"
        );
    }

    #[test]
    fn disassembles_a_known_code_image() {
        // a9 00     lda #$00
        // 85 02     sta $02
        // ad 20 d0  lda $d020
        // d0 fb     bne (-5)      ; from $c007 → $c004
        // 60        rts
        let code = [0xA9, 0x00, 0x85, 0x02, 0xAD, 0x20, 0xD0, 0xD0, 0xFB, 0x60];
        let text = disassemble(&code, 0xC000, "t.prg", code.len() + 2);
        assert!(text.starts_with("; asm: 65c02 acme\n"), "{text}");
        assert!(text.contains("* = $c000"), "{text}");
        assert!(text.contains("lda #$00"), "{text}");
        assert!(text.contains("sta $02"), "{text}");
        assert!(text.contains("lda $d020"), "{text}");
        assert!(text.contains("bne $c004"), "{text}"); // $c007 + 2 - 5
        assert!(text.contains("rts"), "{text}");
        // the address/bytes comment rides along for the reader
        assert!(text.contains("; $c004  ad 20 d0"), "{text}");
    }

    #[test]
    fn undefined_bytes_become_data() {
        // $02 is not a 65C02 opcode → emit it as raw data, keep decoding
        let text = disassemble(&[0x02, 0xEA], 0x0800, "x.prg", 4);
        assert!(text.contains("!byte $02"), "{text}");
        assert!(text.contains("nop"), "{text}"); // $ea decodes fine after it
    }

    #[test]
    fn instruction_running_off_the_end_is_data_not_a_panic() {
        // a lone $AD (LDA abs) with no operand bytes must not read past the end
        let text = disassemble(&[0xAD], 0x0800, "x.prg", 3);
        assert!(text.contains("!byte $ad"), "{text}");
    }
