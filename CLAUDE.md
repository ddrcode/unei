...
- `lsp/` — hand-rolled rust-analyzer client (reader thread + channel, utf-8 positions = byte columns); `editor/analyzer.rs` is the glue (byte↔char conversion lives ONLY there). Main loop polls at 30ms for server messages.
- `term.rs` also hosts the custom ratatui backend: RAPID_BLINK/SLOW_BLINK modifier bits are repurposed as red-straight/yellow-curly diagnostic underlines (Kitty SGR 4:3 + 58).
