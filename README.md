...
- **rust-analyzer**: diagnostics with straight-red / **curly-yellow** underlines, end-of-line ghost text, hover (`K`), goto definition (`gd`), code actions, rename across the workspace (`Space c n`, pre-filled prompt, `:wa` to write it all), macro expansion, and a line-scope type annotator (`gK`) with no nvim equivalent
