...
- **Live grep** (`Space g`): regex-search every line in the project and jump to the match — in-process (no `rg` subprocess), with a context preview
- **Search**: incremental `/` `?`, `n`/`N`, `*`, hlsearch, `:s` substitution — one dialect, Rust regex, smartcase
- **Tree-sitter highlighting**: Rust, Markdown (with fenced-code injection), TOML, YAML, JSON, JS, HTML, Bash, Nix, Python, CSS, justfiles (recipe bodies highlighted as bash, or as the shebang's language), and a bespoke **6502/ACME** assembler grammar — material oceanic theme
- **rust-analyzer**: diagnostics with straight-red / **curly-yellow** underlines, end-of-line ghost text, hover (`K`), goto definition (`gd`), references and a workspace diagnostics list as fuzzy-filterable location lists (`Space c r`, `Space d d`), code actions, rename across the workspace (`Space c n`, pre-filled prompt, `:wa` to write it all), macro expansion, and a line-scope type annotator (`gK`) with no nvim equivalent
