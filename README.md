...
Best experienced in Kitty. On macOS add `macos_option_as_alt left` to kitty.conf, otherwise Option composes characters and the `Alt` chords (resize, split-and-project) never reach the editor. Under tmux, italics and instant-Esc depend on your tmux terminfo (`tmux-256color` recommended).
